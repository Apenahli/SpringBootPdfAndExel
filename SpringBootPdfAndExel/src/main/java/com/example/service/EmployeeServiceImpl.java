...
public class EmployeeServiceImpl implements EmployeeService {
	...
	@Override
	public boolean createExel(List<Employee> employees, ServletContext context, HttpServletRequest request,
		...
		try {
			...
			for (Employee employee : employees) {
				...
				phoneNumberValue.setCellValue(employee.getEmail());
	...
	/*
	 * @Override public boolean createPdf(List<Employee> employees, ServletContext
	 ...
	 * File(filePath); boolean exists = new File(filePath).exists(); if (!exists) {
	 * new File(filePath).mkdirs(); }
	 * 
	 * PdfWriter writer = PdfWriter.getInstance(document, new FileOutputStream(file
	 * + "/" + "employees" + ".pdf")); document.open();
	 * 
	 ...
	 * lastName.setBorderColor(BaseColor.BLACK); lastName.setPaddingLeft(10);
	 ...
	 * email.setBorderColor(BaseColor.BLACK); email.setPaddingLeft(10);
	 ...
	 * 
	 * PdfPCell emailValue = new PdfPCell(new Paragraph(employee.getEmail(),
	 ...
	 * 
	 * PdfPCell phoneNumberValue = new PdfPCell(new
	 * Paragraph(employee.getPhoneNumber(), tableBody));
	 ...
	 * }
	 * 
	 * document.add(table); document.close(); writer.close(); return true;
	 * 
	...
	public List<Employee> search(String searchText) {
		...
		return (List<Employee>) employeeRepository.search(searchText);
	}
}
